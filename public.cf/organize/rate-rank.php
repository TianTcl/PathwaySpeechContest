			...
			function gradefx() {
				...
				var initiate = function() {
					if (!sv.inited) {
						sv.inited = true;
						// Fetch list
						$.get(cv.APIurl+"?app=rank&cmd=list", function(res, hsc) {
				...
				var getScore = function(encID) {
					...
					$.post(cv.APIurl, {app: "grade", cmd: "load", attr: encID}, function(res, hsc) {
						...
						if (dat.success) {
							...
							// Show VDO
							document.querySelector("main .app .slot.video div.wrapper iframe").src = "https://inf.bodin.ac.th/e/Pathway-Speech-Contest/resource/upload/video?view=ID"+btoa(encID);
						} else app.ui.notify(1, dat.reason);
				...
				var saveScore = function() {
					...
					function go_on() {
						...
						if (next.pass) {
							...
							$.post(cv.APIurl, {app: "grade", cmd: mode, attr: next.info }, function(res, hsc) {
								var dat = JSON.parse(res);
								if (dat.success) {
