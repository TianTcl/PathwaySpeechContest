...
// Private
const app_name = "TianTcl's website",
	color_light = "#FFFFFF", color_dark = "#000000";
function private() {
	...
	var context_menu_program = function() {
		...
		try {
			//let selector = 'body *:not(textarea):not(input[type="text"]):not(input[type="number"]):not(input[type="tel"]):not(input[type="email"]):not(input[type="url"])';
			$("body *").on("contextmenu", function(event) { on_cm(event); return false; });
			// $('textarea, type="text", input[type="number"], input[type="tel"], input[type="email"], input[type="url"]').on("contextmenu", function() { return true; });
